...
This is a comprehensive list of all forms on the website.
...
Fields:
- `username` Text - The username to register a new user with.
...
Fields:
- `title` Text - Short title of the project.
- `description` Textarea - Brief description of the project.
- `thumbnail` File - Image to use a main thumbnail of the project.
- `category`Select - A category the project belongs into. Must be selected before slug.
- `slug` Text - A unique end of the proejct's URL.
- `markdown-article` Textarea - Article of the project in markdown.
- `gallery-upload[]` File[] - Images to use in the article.
- `gallery-uuid[]` Hidden[] - UUID generated by the browser used as a file name for the image.
- `link-url[]` Text[] - URLs of links provided with the project.
...
Fields:
- `user-action` Select - Select an option to run an action on a user.
- `new-password` Password - Set a new password for the user.
- `confirm-password` Password - Confirm the user's new password.
...
- Project Upload
    ...
    - Their file name is based on the uploaded file's name, but corrected to be safe and ensured to be unique by appending numbers when collisions occur.
...
- Project Gallery Image
    - Additional image files in `.webp` or `.gif` format uploaded by the user to accompany their project's article.
    - Their file name is a UUID without an extension.
